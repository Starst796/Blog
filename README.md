...
| **可视化后台** | 登录 `/admin` 即可增删改文章、项目与站点信息，支持 Markdown 实时预览（在浏览器本地渲染，编辑过程不发请求）与图片上传 |
| **草稿与敏感内容** | `draft: true` 表示「还没写完」，`sensitive: true` 表示「永不对外输出」，两者对外都是 404 |
| **本地草稿兜底** | 编辑器在预览渲染完成后把字段与正文写进 `localStorage`，保存到服务器即清除；误关标签页、刷新也能捞回来 |
| **文集与标签** | `collection: 名字` 把文章归入文集（留空即「未归档」），列表页按发布时间（日期 + 时刻）倒序平铺并标注文集与标签，详情页的「更早 / 更新」只在同一文集内跳转 |
...
title: 标题
date: 2026-09-10
time: "14:32"            # 可选，发布时刻，用于同日多篇的排序
updated: 2026-09-20        # 可选
summary: 一句话摘要         # 留空则自动截取正文首段
collection: 我的博客搭建记   # 可选，留空即「未归档」
tags: [Flask, 随笔]
...
- **默认为「未归档」**：没写 `collection` 的文章都在这个文集里，无需额外配置。
- **列表页按时间倒序平铺**：`/articles` 保持按「发布时间」从新到旧的原有顺序，
...
- **排序依据**：文章按 `date + time` 从新到旧排列；时刻相同时按 slug 倒序，顺序稳定。
- **默认值**：后台新建文章时，时刻默认填入**此刻**（取整到分钟），也可改成任意时间。
- **老文章**：没有 `time` 字段的文章一律当作 **00:00**，因此同日多篇的先后关系
  与加字段之前完全一致（等于只按日期排）。后台编辑老文章时表单会回填 `00:00`，
  原样保存**不会**往文件里写入多余的 `time` 字段。
- **务必加引号**：`time: "14:32"`。YAML 1.1 会把裸写的 `14:32` 当成**六十进制整数**
  （解析成 `750`），而 `09:30` 因为首位是 `0` 反而仍是字符串——行为不一致。
  后台保存时写出的值会被 PyYAML 自动加引号，手写时请自行补上。
...
- **内容**：文章与项目的新建 / 编辑 / 删除，含 slug 重命名（旧文件自动清理）；
  内容列表两张表各自分页（每页 10 篇文章 / 5 个项目），删除后停在本页不会跳回第 1 页
- **写文章 / 新建项目**：Markdown 实时预览、一键上传图片并插入链接。预览由浏览器本地渲染（`static/js/markdown-local.js`），整篇正文不出浏览器，出图速度只取决于本机；代价是预览与线上最终排版可能略有出入，写列表时记得在前面留一个空行——线上启用了 `sane_lists`，紧跟段落的 `1.` 不会变成列表
- **站点信息**：名称、标语、简介、所在地、邮箱、头像、社交链接
- **预览草稿与敏感内容**：在公开页面上直接看到未发布内容
- **本地草稿兜底**：编辑中的字段与正文存在浏览器里，保存到服务器后自动清除
...
### 编辑器里的本地草稿（localStorage）

编辑到一半关掉标签页、刷新，或者浏览器崩了，写好的正文不该跟着消失。带编辑器的页面
（文章 / 项目表单）会在**预览渲染完成**的那一刻，把整份草稿——所有表单字段加正文
——写进 `localStorage`，记录键就是页面路径（如
`editor-cache:/admin/articles/<slug>/edit`）。

- **什么时候写**：跟着预览的节拍走。与打开页面时的那份内容一致就不写，改回原样还会
  把记录删掉，所以只有「动过」才留痕迹。
- **什么时候删**：保存成功（重定向回来时带着成功提示）、在提示条或内容列表上点
  「丢弃」、超过 7 天。保存失败会带错误提示回到编辑页，记录继续留着。
- **怎么恢复**：再次打开这个编辑页时，页顶提示「恢复草稿 / 不用了」；内容列表里对应
  的行会标出「本地草稿 · 多久以前」，可以直接「恢复」或「丢弃」。在新建页写的草稿
  还没有对应的内容，会单列在「浏览器里的未保存草稿」一节里。
- **边界**：只存在这一个浏览器里，换浏览器、换机器都看不到；它是「这一份的兜底」，
  不是同步方案，正文仍以服务器上的文件为准。

### 编辑器预览里的图片占位

预览是整篇重绘的（重绘一次就换掉整棵 DOM），图片若「先是 0 高、解码完再变高」，
预览的高度会连着变好几次，滚动位置跟着被拽走——插入一张图时尤其明显。
解决办法是让图片的尺寸**在解码之前就是已知的**：

- 上传接口在返回 `url` 的同时返回 `width` / `height`（后端校验文件时本就用 PIL
  解过一次图，顺手读出尺寸，没有额外开销）；
- 打开编辑页时再取一次存量清单（`GET /admin/uploads/sizes`，仅登录后可访问），
  它覆盖的是**以前上传的图**——旧文章里的图恰恰是日常编辑时最常碰到的那些；
- 编辑器把「图片地址 → 尺寸」记进 `sessionStorage` 的 `editor-image-size`，
  预览每次重绘后立刻按它给 `<img>` 补上 `width` / `height`，浏览器据此先撑出
  正确大小的占位盒，解码前后高度一致。

正文里有 `uploads` 图片时，第一遍预览会**等清单回来再画**（最多等 1.5 秒，超时或
接口出错就直接画）——不先取的话，第一遍画出来的旧图全是 0 高，等图片解码完高度再
长回来，滚动位置已经被顶走一次了。清单只放在内存里，不进 sessionStorage。

两个来源都没有的图（外链图）仍按老样子等解码（也就是回退行为）。作者自己写了
`width` / `height` 的图片不会被覆盖。关掉标签页记录即失效，不会长期堆积。
